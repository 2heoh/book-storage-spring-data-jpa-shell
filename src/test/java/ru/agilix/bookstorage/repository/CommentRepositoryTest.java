...
//                .Book(Create.Book(2).build())
    ...
    void shouldGetAllCommentsByBookId() {
        ...
        assertThat(comments)
                ...
                .allMatch(c -> c.getBook().getTitle().equals("Война и мир"));
    }
    ...
    void shouldReturnCommentsInReverseDateOrder() {
        ...
        for (Comment c : expectedOrder){
            c.setBook(booksRepository.findById(1L).get());
        }
    ...
    @Test
    void shouldReturnEmptyIfPassedNonExitingId() {

        final var book = commentRepository.findCommentByBook_Id(-1L);

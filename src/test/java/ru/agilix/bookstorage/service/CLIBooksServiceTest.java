...
class CLIBooksServiceTest {
    ...
    void shouldShowExistingBookById() {
        ...
        given(booksRepository.findById(1L)).willReturn(java.util.Optional.ofNullable(book));
    ...
    void shouldUpdateBook() {
        given(booksRepository.findById(anyLong())).willReturn(java.util.Optional.ofNullable(Create.Book().Id(1L).Title("title").build()));
    ...
    void shouldDeleteBookWhenBookExists() {
        ...
        given(booksRepository.findById(1L)).willReturn(java.util.Optional.of(bible));

        service.deleteBook(1L);

        verify(booksRepository, times(1)).delete(bible);
        verify(output, times(1)).showBookDeletedMessage(1L);
        verify(output, times(0)).showBookNotFound(anyLong());
    }
    ...
    void deletingNonExistingBookDisplaysBookNotFoundMessage() {
        ...
        verify(output, times(1)).showBookNotFound(-1L);
    }
    ...
    void shouldShowCommentsByBookId() {
        ...
        given(booksRepository.findById(1L)).willReturn(java.util.Optional.of(book));

        service.getCommentsByBookId(1L);
